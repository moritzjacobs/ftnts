<?php
namespace moritzjacobs\Ftnts\PluginBase;
...
class Main {
	...
	public function __construct($prefs, $base_location) {
		...
		$this->slug = Utils::sluggify($class_name);
	...
	private function admin_js($files) {
		...
		if (true) {
			foreach($files as $file) {
				$js_slug = Utils::sluggify(str_replace(".js", "", basename($file)));
				add_action( 'admin_enqueue_scripts', function() use ($file, $slug, $js_slug, $version){
					$handle = $slug .'-admin-'.$js_slug;
					wp_register_script($handle, plugins_url($file, dirname(__FILE__)), array(), $version);
	...
	private function public_js($files) {
		...
		foreach($files as $file) {
			$js_slug = Utils::sluggify(str_replace(".js", "", basename($file)));
			add_action( 'wp_enqueue_scripts', function() use ($file, $slug, $js_slug, $version){
				wp_enqueue_script($slug .'-admin-'.$js_slug, plugins_url($file, dirname(__FILE__)), array(), $version, true);
			});
		}
	}
